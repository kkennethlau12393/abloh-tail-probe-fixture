...
test("the closed vocabulary passes through, and nothing else is invented from a sentence", () => {
  ...
  /* AND THE SPACED SENTENCE IS STILL GENERALISED RATHER THAN GUESSED AT. It was this product's own
     until 2026-08-30 - `apps/cli/src/index.ts` wrote it at two sites and the schema's `reason` was
     typed `string`, so nothing stopped it; the type is the union now and only a genuinely older CLI
     can send this. The downgrade stays because losing one sentence of precision is the right price
     for not discarding a complete measurement. */
